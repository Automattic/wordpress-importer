...
class WP_REST_Imports_Controller extends WP_REST_Posts_Controller {
...
//				'show_in_index' => false,
	...
	function add_piece( $request ) {
		...
		if ( ! update_post_meta( $post_id, 'wp_file_import_piece_' . $piece_id, $piece ) ) {
	...
	function import_from_pieces( $request ) {
		...
		if ( 'wp_file_import' !== $post_obj->post_type ) {
		...
		for ( $i = 0; $i < $num_pieces; $i++ ) {
			$piece = get_post_meta( $post_id, "wp_file_import_piece_$i", true );
			if ( ! $piece ) {
